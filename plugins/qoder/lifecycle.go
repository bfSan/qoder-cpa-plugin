...
func applyExhaustedPolicy(authIndex, authID string, sa *storedAuth, cr *creditsSummary, reason string) error {
	...
	action := lifecycleActionFor(authRegion(sa), cr)
	switch action {
...
func listEntryMatchesUID(f pluginapi.HostAuthFileEntry, uid, wantName string) bool {
	...
	return strings.EqualFold(base, providerName+"-"+uid)
}
...
// displayEmailForAuth maps the Qoder nickname into CPA's email slot.
//
// CPA v7.3.7's native auth card chooses its title from email -> project_id ->
// filename and ignores AuthData.Label. Neither Qoder region exposes a usable
// email in this flow, so the nickname is the only reliable display identity.

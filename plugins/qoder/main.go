...
const (
	...
	defaultUpstreamBaseIntl = "https://openapi.qoder.sh"
	gatewayBaseIntl         = "https://api3.qoder.sh"
	clientUA                = "Go-http-client/2.0"

	// Auth endpoints (PAT → jobToken exchange + refresh).
	endpointJobTokenExchange = defaultUpstreamBaseCN + "/api/v1/jobToken/exchange"
	endpointJobTokenRefresh  = defaultUpstreamBaseCN + "/api/v1/jobToken/refresh"

	// Business endpoints (jt- Bearer, no COSY).
	endpointUserInfo      = defaultUpstreamBaseCN + "/api/v1/userinfo"
	endpointQuotaUsage    = defaultUpstreamBaseCN + "/api/v2/quota/usage"
	endpointUserPlan      = defaultUpstreamBaseCN + "/api/v2/user/plan"
	endpointCheckinStatus = defaultUpstreamBaseCN + "/sash/api/v1/me/daily-check-in/status"
	endpointCheckinClaim  = defaultUpstreamBaseCN + "/sash/api/v1/me/daily-check-in/claim"
	endpointProUpgrade    = defaultUpstreamBaseCN + "/sash/api/v1/me/pro-upgrade/claim"

...
| 能力 | 说明 |
|---|---|
| **双区域 + 双登录方式** | OAuth 面板可分别登录 CN（qoder.com.cn）与 Intl（qoder.com）；也支持 PAT 导入（pt-，长期有效兜底）。每个账号按 auth 文件内的 `region` 独立路由 |
...
| **积分面板** | 账号卡片：昵称/积分/计划/签到状态/操作（签到/刷新/选用），并展示每个 `(账号, 模型)` 的冷却状态 |
| **模型级冷却** | 上游 429 或空流失败只冷却失败的 `(auth, model)`，同账号的其他模型继续可用；面板可单独解除 |

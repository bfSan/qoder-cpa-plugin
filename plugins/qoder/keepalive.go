...
func refreshCall(sa *storedAuth) (json.RawMessage, []byte, int, error) {
	...
	if strings.HasPrefix(sa.Auth.RefreshToken, "drt-") {
		...
		data, status, err := doRawJSON(sharedHTTPClient(), http.MethodPost, upstreamBaseFor(sa)+"/api/v1/deviceToken/refresh", nil, bytes.NewReader(body))
		if err == nil {
	...
	body, _ := json.Marshal(map[string]string{"refresh_token": sa.Auth.RefreshToken})
	data, status, err := doRawJSON(sharedHTTPClient(), http.MethodPost, endpointJobTokenRefreshFor(sa), nil, bytes.NewReader(body))
	if err == nil {
		return data, data, status, nil
	}
	...
	if sa.Auth.PersonalToken != "" {
		...
		data2, status2, err2 := doRawJSON(sharedHTTPClient(), http.MethodPost, endpointJobTokenExchangeFor(sa), nil, bytes.NewReader(patBody))
		if err2 == nil {
			return data2, data2, status2, nil
		}
	}

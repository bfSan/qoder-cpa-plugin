...
func TestClassify(t *testing.T) {
	cases := []struct {
		...
		{200, `{"code":4008,"msg":"Your requests have exceeded the quota"}`, ErrPlanLimit},
		{429, ``, ErrSoftRate},
...
func TestCheckinReqSourceProbeOn9074(t *testing.T) {
	// v0.12.41/43: 9074 = 活动校验拒绝（req_source 与 token 产品谱系错配）→
	// 按 variant 序列换 body 重试（cn: 1→官方空 body→2）；全组合被拒才把
	// 9074 作为 soft rate-limit 上抛，且错误携带探测诊断（variant/device_id_set）。
	a := &auth.Auth{AccessToken: "tok", Variant: "cn"}
	var bodies []string
	n := 0
	c := testClient(func(r *http.Request) (*http.Response, error) {
		n++
	...
	if n != 3 || bodies[0] != `{"req_source":1}` || bodies[1] != `{}` || bodies[2] != `{"req_source":2}` {
		t.Errorf("probe order = %v (%d attempts), want [req_source:1 empty req_source:2]", bodies, n)
	}
	...
	// 全组合均被拒（真·活动侧拒绝）→ 每个 body 一次 Cloud-IDE-JWT 尝试
	// （9074 不在同源内换鉴权方案），错误仍以 biz 9074 上抛供调度器退避重试，
	// 且消息携带 v0.12.43 探测诊断（组合清单 + variant + device_id_set）。
	n2 := 0
	...
	if n2 != 3 {
		t.Errorf("attempts=%d, want 3 (one per body in cn probe list)", n2)
	}
	var ue *Error
...
func TestCheckinReqSourceSoloVariantFirst(t *testing.T) {
	// v0.12.43: solo 谱系官方（TraeWork）发 req_source=2 —— 序列首位必须是 2，
	// 命中即止（不再像固定 [1,2] 序列那样先打一发跨谱系空炮）。
	a := &auth.Auth{AccessToken: "tok", Variant: "solo"}
	var bodies []string
	n := 0
	c := testClient(func(r *http.Request) (*http.Response, error) {
		n++
	...
	if n != 1 || bodies[0] != `{"req_source":2}` {
		t.Errorf("solo probe = %v (%d attempts), want first-probe req_source:2", bodies, n)
	}
	if resp.ReqSourceUsed != `{"req_source":2}` {
		t.Errorf("ReqSourceUsed=%q, want req_source:2", resp.ReqSourceUsed)
	}
}
